...
class KDTree
{
  ...
  private:
     ...
     */
    void nodeToVec( vector<Point<Dim>> newPoints, KDTreeNode* myNode, int currDimensions );
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode* currentNode, int currDimensions) const;
};
