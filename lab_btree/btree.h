...
size_t search_idx(unsigned front, unsigned end, const std::vector<T>& elements, const C& val)
{          
    if( (end - front) == 0 ){
        if( val > elements[end] )
            return end + 1;
    ...
    unsigned int split = (end + front) / 2;
    if( elements[split] > val )
        return search_idx(0, split, elements, val);
    if( elements[split] < val )
        return search_idx(split + 1, end, elements, val);
    else
        return split;
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    if( elements.size() == 0 )
        return 0;
    return search_idx( 0, elements.size()-1, elements, val );
}
